...
def load(p_no, mode):
    ...
    if(mode != 'inference'):
        ...
        source_file_names = dataset_inf[0]
        target_file_names = dataset_inf[1]
        source_image_names = dataset_inf[2]
        target_image_names = dataset_inf[3]
        transforms = np.float32(dataset_inf[4:])

        target_container = np.zeros((1, IMG_HT, IMG_WDT, 1), dtype = np.float32)
        source_container = np.zeros((1, IMG_HT, IMG_WDT, 1), dtype = np.float32)
        source_img_container = np.zeros((1, IMG_HT, IMG_WDT, 3), dtype = np.float32)
        target_img_container = np.zeros((1, IMG_HT, IMG_WDT, 3), dtype = np.float32)
        transforms_container = np.zeros((1, 4, 4), dtype = np.float32)

        s_name = source_file_names
        t_name = target_file_names
        img_source_name = source_image_names
        img_target_name = target_image_names
        transform = transforms

        print(source_file_names)

        warped_ip = np.float32(smc.imread(s_name, True))[300:675, 19:1261]
        warped_ip[0:5,:] = 0.0 ; warped_ip[:,0:5] = 0.0 ; warped_ip[IMG_HT - 5:,:] = 0.0 ; warped_ip[:,IMG_WDT-5:] = 0.0 ;
        warped_ip = (warped_ip - 40.0)/40.0
        source_container[0, :, :, 0] = warped_ip

        target_ip = np.float32(smc.imread(t_name, True))[300:675, 19:1261]
        target_ip[0:5,:] = 0.0 ; target_ip[:,0:5] = 0.0 ; target_ip[IMG_HT - 5:,:] = 0.0 ; target_ip[:,IMG_WDT-5:] = 0.0 ;
        target_ip = (target_ip - 40.0)/40.0
        target_container[0, :, :, 0] = target_ip

        source_img = np.float32(smc.imread(img_source_name))[300:675, 19:1261, :]
        source_img[0:5,:,:] = 0.0 ; source_img[:,0:5,:] = 0.0 ; source_img[IMG_HT - 5:,:,:] = 0.0 ; source_img[:,IMG_WDT-5:,:] = 0.0 ;
        source_img = (source_img - 127.5)/127.5
        source_img_container[0, :, :, :] = source_img

        target_img = np.float32(smc.imread(img_target_name))[300:675, 19:1261, :]
        target_img[0:5,:,:] = 0.0 ; target_img[:,0:5,:] = 0.0 ; target_img[IMG_HT - 5:,:,:] = 0.0 ; target_img[:,IMG_WDT-5:,:] = 0.0 ;
        target_img = (target_img - 127.5)/127.5
        target_img_container[0, :, :, :] = target_img

        transforms_container[0, :, :] = np.linalg.inv(transform.reshape(4,4))

        source_container = source_container.reshape(1, 1, IMG_HT, IMG_WDT , 1)
        target_container = target_container.reshape(1, 1, IMG_HT, IMG_WDT , 1)
        source_img_container = source_img_container.reshape(1, 1, IMG_HT, IMG_WDT, 3)
        target_img_container = target_img_container.reshape(1, 1, IMG_HT, IMG_WDT, 3)
        transforms_container = transforms_container.reshape(1, 1, 4, 4)
